...
$imageType       = $this->params['configs']->get('jg_detail_view_type_image', 'detail', 'STRING');
$showDescription = $this->params['configs']->get('jg_detail_view_show_description', 0, 'INT');
$showTags        = $this->params['configs']->get('jg_detail_view_show_tags', 0, 'INT');
$showMetadata    = $this->params['configs']->get('jg_detail_view_show_metadata', 0, 'INT');
...
$canCheckin = $this->getAcl()->checkACL('editstate', 'com_joomgallery.image', $this->item->id, $this->item->catid, true)
  || $this->item->checked_out == $this->getCurrentUser()->id;

$imageUrl     = JoomHelper::getImg($this->item, $imageType);
...
// Flatten stored metadata so one useful camera field can be shown inline and
// all remaining fields can be presented in the modal.
...
<?php foreach(ModuleHelper::getModules('jg_image_top') as $module) : ?>
...
<article class="jg-detail overflow-hidden" itemscope itemtype="https://schema.org/ImageObject">
  <div class="jg-detail__stage position-relative d-flex align-items-center justify-content-center p-3 p-md-5">
    <a class="jg-detail__back btn btn-light rounded-circle position-absolute top-0 start-0 m-3 shadow" href="<?php echo htmlspecialchars($this->backUrl, ENT_QUOTES, 'UTF-8'); ?>" aria-label="<?php echo Text::_('JBACK'); ?>" title="<?php echo Text::_('JBACK'); ?>"><span class="icon-arrow-left" aria-hidden="true"></span></a>
    <?php if($previousUrl) : ?>
      <a class="jg-detail__nav jg-detail__nav--prev btn btn-light rounded-circle position-absolute top-50 start-0 translate-middle-y ms-2 shadow" href="<?php echo $previousUrl; ?>" aria-label="<?php echo Text::_('JPREV'); ?>"><span class="icon-chevron-left" aria-hidden="true"></span></a>
    <?php endif; ?>
    ...
    <?php if($nextUrl) : ?>
      <a class="jg-detail__nav jg-detail__nav--next btn btn-light rounded-circle position-absolute top-50 end-0 translate-middle-y me-2 shadow" href="<?php echo $nextUrl; ?>" aria-label="<?php echo Text::_('JNEXT'); ?>"><span class="icon-chevron-right" aria-hidden="true"></span></a>
    <?php endif; ?>
  </div>

  <div class="container-fluid py-4">
    <div class="d-flex flex-wrap align-items-center gap-2 pb-3 border-bottom" aria-label="Image actions">
      <button class="btn btn-outline-secondary" type="button" disabled title="Favorites are coming soon"><span class="icon-heart me-1" aria-hidden="true"></span>Favorite</button>
      <button class="btn btn-outline-secondary" type="button" disabled title="Comments are coming soon"><span class="icon-comment me-1" aria-hidden="true"></span>Comment</button>
      <button class="btn btn-outline-secondary" type="button" disabled title="Downloads are coming soon"><span class="icon-download me-1" aria-hidden="true"></span>Download</button>
      <button class="btn btn-outline-secondary ms-md-auto" type="button" data-jg-copy-link><span class="icon-link me-1" aria-hidden="true"></span><span>Copy link</span></button>
    ...
    <?php if($showTags && trim($tags) !== '') : ?><div class="d-flex flex-wrap gap-2 py-4 jg-detail__tags"><?php echo $tags; ?></div><?php endif; ?>
    <?php if($showDescription && trim((string) $this->item->description) !== '') : ?><div class="lead mb-4" itemprop="description"><?php echo JoomHelper::sanitizeHtml($this->item->description); ?></div><?php endif; ?>
    ...
    <?php if($showMetadata && !empty($otherMetadata)) : ?>
